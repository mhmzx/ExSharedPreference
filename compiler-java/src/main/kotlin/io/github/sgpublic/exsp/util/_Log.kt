...
interface Logable: AnnotationMirror, Element {
    ...
    override fun getAnnotationType(): DeclaredType {
        return ExPreferenceProcessor.getType(targetAnnotation.canonicalName)
    }
...
class Logger(private val logable: Logable) {
    fun trace(msg: CharSequence) {
        ExPreferenceProcessor.mMessager.printMessage(Diagnostic.Kind.OTHER, msg, logable, logable)
    }
    fun info(msg: CharSequence) {
        ExPreferenceProcessor.mMessager.printMessage(Diagnostic.Kind.NOTE, msg, logable, logable)
    }
    fun warn(msg: CharSequence) {
        ExPreferenceProcessor.mMessager.printMessage(Diagnostic.Kind.WARNING, msg, logable, logable)
    }
    fun error(msg: CharSequence) {
        ExPreferenceProcessor.mMessager.printMessage(Diagnostic.Kind.ERROR, msg, logable, logable)
    }
}
